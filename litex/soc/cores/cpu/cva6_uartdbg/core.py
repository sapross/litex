...
GCC_FLAGS = {
    ...
    #                       imacfd
    "standard": "-march=rv64i2p0_mac_zicsr_zifencei -mabi=lp64 ",
    "standard32": "-march=rv32imac_zicsr_zifencei -mabi=ilp32 ",
    "full": "-march=rv64gc_zicsr_zifencei   -mabi=lp64 ",
}
            ...
            if res and not re.match("//", l):
                ...
                if re.match("\+incdir\+", l):
                    platform.add_verilog_include_path(
                        os.path.join(basedir, res.group(2))
                    )
                else:
                    ...
                    if True:  # TODO: other FPGAs
                        if filename.endswith("tc_sram_wrapper.sv"):
                            filename = filename.replace(
                                "tc_sram_wrapper.sv", "tc_sram_fpga_wrapper.sv"
                            )
                            platform.add_source(
                                os.path.join(
                                    basedir,
...
class CVA6_UARTDBG(CPU):
    ...
    def __init__(self, platform, variant="standard"):
        ...
        wrapper_root = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "cva6_wrapper"
        )
        ...
        add_manifest_sources(
            platform,
            os.path.join(get_data_mod("cpu", "cva6").data_location, "core", manifest),
        )
        # Add wrapper sources
    ...
    def add_uart(self, pads):
        self.debug_rx = Signal()
        self.debug_tx = Signal()

        self.cpu_params.update(
            i_debug_rx=self.debug_rx,
            o_debug_tx=self.debug_tx,
        )
    ...
    def set_reset_address(self, reset_address):
        ...
        ), "cpu_reset_addr hardcoded in during elaboration!"
